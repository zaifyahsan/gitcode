...
def mapper( ranges ):
	...
	model = classifiers.LogisticRegression( ranges[1], ranges[2], opti_learning_rate = 1e-3 ) #, normalize_laplacian=True )

	print 'model created'

	# create distance matrix
	adj_matrix = species.ExponentialAdjacencyMatrixBuilder( ranges[0] )
	species_list, A = adj_matrix( ranges[9] )

	print 'distance matrix created '

	# learn model
	model.fit( ranges[3], ranges[4], ranges[5], ranges[6], A, species_list ) # self.reqxtrain, self.reqx_speciestrain, self.reqytrain, self.reqxorthodicttrain, A, species_list )

	print 'model learned'

#	model = LogisticRegression( penalty= 'l2', C = 1e9 )
#	model.fit( ranges[3], ranges[5] )
	# test model
	ypred = model.predict( ranges[7] ) #self.reqxvalid )

	print 'model tested'

	# return ranges and auc
	#compute auc score
	fpr, tpr, thresholds = roc_curve( ranges[8], ypred )
	roc_auc = auc(fpr, tpr)

	print 'auc: ', roc_auc
	returnranges = [ ranges[0], ranges[1], ranges[2], roc_auc ]
